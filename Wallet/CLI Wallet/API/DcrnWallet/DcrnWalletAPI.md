...
**Note**: The naming style used for autogenerated identifiers may differ depending on the language being used. 

The server may be running without a loaded wallet, in which case the Wallet service is not running and the Loader service must be used to create a new or load an existing wallet.
...
The short examples of method can be found [here](./DcrnWalletAPI_Usage#Version).
...
**Expected errors:** None
The short examples of method can be found [here](./DcrnWalletAPI_Usage#WalletExists).
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default dcrnd RPC port of the
...
- [DcrnWallet API](#dcrnwallet-api)
  ...
  - [`WalletService`](#walletservice)
      ...
      - [`Balance`](#balance)
      - [`BlockInfo`](#blockinfo)
      - [`GetAccountExtendedPubKey`](#getaccountextendedpubkey)
      - [`GetAccountExtendedPrivKey`](#getaccountextendedprivkey)
    ...
    - [Shared messages](#shared-messages)
      ...
      - [`FetchHeadersNotification`](#fetchheadersnotification)
      - [`FetchMissingCFiltersNotification`](#fetchmissingcfiltersnotification)
      - [`RescanProgressNotification`](#rescanprogressnotification)
  ...
  - [`AccountMixerService`](#accountmixerservice)
    - [Methods](#methods-5)
